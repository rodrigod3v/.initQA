export const httpRequestStyles = {
    ...
    // Controls - Aligned with sidebarHeader and resultsHeader (h-12)
    controls: "h-12 border-b border-main/10 flex items-center bg-deep/20 shrink-0 relative z-20 px-4",
    urlBar: "flex flex-1 items-center gap-2",
    methodSelect: "bg-surface border-sharp border-main/20 px-3 h-8 font-mono text-[11px] text-accent focus:outline-none focus:border-accent/40 w-auto shrink-0",
    protocolSelect: "bg-deep border-sharp border-main/20 px-3 h-8 font-mono text-[10px] text-secondary-text focus:outline-none focus:border-accent/40 w-auto uppercase tracking-widest shrink-0",
    urlInput: "flex-1 bg-surface border-sharp border-main/20 px-3 h-8 font-mono text-xs text-primary-text focus:outline-none focus:border-accent/40 placeholder:opacity-20 min-w-0 flex-shrink",
    actionButton: "h-8 px-4 text-[10px] uppercase tracking-[0.2em] w-auto",
