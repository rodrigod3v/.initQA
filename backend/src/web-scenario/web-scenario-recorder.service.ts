...
export class WebScenarioRecorderService {
  private activeSessions = new Map<string, { browser: any, context: BrowserContext, page: Page, steps: any[] }>();
  ...
  async startRecording(url: string, sessionId: string) {
    ...
    await page.addInitScript(() => {
      window.addEventListener('click', (e) => {
        ...
        if (target.tagName === 'INPUT' && (target as HTMLInputElement).type === 'text') return; // Handled by input/change
        
        // --- SMART SELECTOR LOGIC ---
        // Search for the closest meaningful interactive element
        const interactive = target.closest('button, a, input, [role="button"], [onclick]') || target;
      ...
      function getSelector(el: HTMLElement): string {
        ...
        if (attr('placeholder')) return `[placeholder="${attr('placeholder')}"]`;
        ...
        if (el.classList.length > 0) {
          const classes = Array.from(el.classList)
            .filter(c => !c.includes(':') && !c.includes('[') && c.length < 50)
            .join('.');
  ...
  private processSteps(rawSteps: any[]) {
    ...
    for (let i = 0; i < rawSteps.length; i++) {
        ...
        if (current.type === 'FILL' && next && next.type === 'FILL' && next.selector === current.selector) {
            continue;
        }
